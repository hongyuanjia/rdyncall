...
this_file <- if (length(file_arg)) sub("^--file=", "", file_arg[[1L]]) else "tools/record-sdl3-demo.R"
...
if (!requireNamespace("asciicast", quietly = TRUE)) {
    stop(
        "tools/record-sdl3-demo.R requires the optional asciicast package. ",
        "Install it with pak::pak(\"r-lib/asciicast\") or remotes::install_github(\"r-lib/asciicast\").",
        call. = FALSE
    )
}

cast_file <- if (length(args) >= 1L) args[[1L]] else file.path(repo, "man", "figures", "sdl3-demo.cast")
svg_file <- if (length(args) >= 2L) args[[2L]] else sub("[.][^.]*$", ".svg", cast_file)
script_file <- file.path(repo, "tools", "asciicast", "sdl3-minimal.R")
