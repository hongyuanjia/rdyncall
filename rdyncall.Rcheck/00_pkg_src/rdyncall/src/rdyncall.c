...
SEXP C_dyncall(SEXP args) /* callvm, address, signature, args ... */
{
  DCCallVM*   pvm;
  void*       addr;
  ...
  for(;;) {
    ...
    if (ptrcnt == 0) { /* base types */
      ...
      switch(ch) {
        ...
        break;
        default: error("Signature type mismatch at position %d: Unknown token '%c'.", argpos, ch); /* dummy */ return R_NilValue;
      }
    } else { /* ptrcnt > 0 */
      DCpointer ptrValue;
