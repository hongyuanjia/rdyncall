...
handling of foreign data types via \code{cdata} and wrapping of R functions as C callbacks via \code{ccallback}.
...
\code{ccallback}. Callback wrappers are defined by a \emph{callback
...
\code{ccallback} returns an external pointer that can
...
tag name. They are wrapped as C callback pointers via \code{ccallback}
