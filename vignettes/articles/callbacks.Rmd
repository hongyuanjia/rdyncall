...
comparison function, completion hook, or any other callback pointer. The main
task is not just creating the pointer; it is also keeping the R function and
its state alive for exactly as long as C can call it.
...
For stored callbacks, make the owner explicit. A convenient rule is: the R
object that represents the foreign registration should also keep the callback
pointer and any R state used by the callback.

```{r callback-owner, eval=FALSE}
...
When the C API provides an unregister or destroy function, call it before
dropping the last R reference to the owner object.
...
Use `tryCatch()` inside callbacks that may run under foreign control. Store the
R error somewhere inspectable, and return a value that the C API documents as a
failure.

```{r callback-errors, eval=FALSE}
last_error <- NULL
...
This pattern does not make all foreign event loops safe, but it prevents an R
condition from being the mechanism that crosses the C callback boundary.
...
For a real API, pair this with the corresponding unregister or destroy function
and clear the R reference only after C no longer holds the pointer.

## Next steps

- Use [signatures](signatures.html) to translate callback prototypes and
  callback pointer arguments.
- Use [structs, unions, and memory](structs-unions-memory.html) when callback
  arguments are raw pointers to C data.
- Use [FFI safety](ffi-safety.html) before registering callbacks that may be
  called asynchronously or from foreign event loops.
- Use [troubleshooting](troubleshooting.html) when a callback crashes, returns
  nonsense, or is called after R state has been cleaned up.
